...
\begin{itemize}
    \item \textbf{Klasa reprezentująca graf oraz jego strukturę (wierzchołki i krawędzie):}
    \texttt{Graph} — przechowuje zbiór wierzchołków (\texttt{HashMap<Short, Vertex>}) oraz krawędzi (\texttt{HashSet<Edge>}), a także obsługuje ich dodawanie i wczytywanie z pliku.

    \item \textbf{Klasa opisująca pojedynczą krawędź grafu:} \texttt{Edge} — zawiera pola: nazwa, wierzchołek startowy (\texttt{u}), końcowy (\texttt{v}) oraz wagę krawędzi.

    \item \textbf{Klasa reprezentująca wierzchołek grafu:}
    \texttt{Vertex} — rekord przechowujący współrzędne (\texttt{x, y}) pojedynczego wierzchołka.

    \item \textbf{Klasa odpowiedzialna za implementację algorytmu wyznaczania współrzędnych wierzchołków:}  \texttt{Graph} — zawiera metodę \texttt{randomizeCoordinates(...)} generator losowych współrzędnych dla wszystkich wierzchołków grafu w zadanym obszarze. Przypisuje każdemu wierzchołkowi nowe pozycje na podstawie pseudolosowych wartości.

    \item \textbf{Klasa odpowiedzialna za interfejs graficzny:}
    \texttt{JavaGUI} — odpowiada za okno aplikacji, przyciski, wybór pliku i integrację z grafem.

    \item \textbf{Klasa odpowiedzialna za rysowanie grafu w komponencie graficznym:}
    \texttt{DrawingPanel} — dziedziczy po \texttt{JPanel} i implementuje metodę rysującą wierzchołki i krawędzie przy użyciu \texttt{Graphics2D}.
\end{itemize}
...
\noindent{}Implementacja metody \verb|Graph.readFromFile|:
...
\subsubsection{Cały graf}
\begin{minted}[linenos]{java}
public class Graph {
	public final HashSet<Edge> edges; //zbiór krawędzi
	public final HashMap<Short, Vertex> vertices; //zbiór wierzchołków, z których każdy jest identyfikowany na podstawie numeru ID
...
Klasa \verb|Vertex| jest zaimplementowana w jednej linijce kodu jako rekord przechowujący pola \verb|x| oraz \verb|y|.
...
\begin{figure}[p!]
    ...
    \caption{Wczytany graf po użyciu algorytmu Tuttego.}
    \label{fig:one}
\end{figure}
...
\begin{figure}[p!]
    ...
    \caption{Panel ustawień do zmieny wyglądu grafu.}
    \label{fig:two}
\end{figure}
